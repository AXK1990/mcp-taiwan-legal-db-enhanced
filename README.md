# mcp-taiwan-legal-db（增補版）🌟

[English](README.en.md) · **繁體中文**

**本專案為 [mcp-taiwan-legal-db](https://github.com/lawchat-oss/mcp-taiwan-legal-db) 的增補版本**
...
而本專案 **mcp-taiwan-legal-db-enhanced** 則沿用原專案的基本架構，再增設下列功能，使 AI 助手可以更完整、更大範圍地取得所需資料：
...
> **註**：以下功能表格中，🌟 標記為本增補版本新增功能，其餘功能均繼承自原專案。
...
如果你之前安裝過原作者的 `mcp-taiwan-legal-db`，建議先移除再安裝增補版，避免套件衝突。
...
> **註**：本增補版本維持原有 8 個工具，並新增 3 個法令判解系統相關工具，共 11 個工具。此外，`search_judgments` 工具已增強支援簡易案件系統查詢、進階搜尋與分頁機制。

11 個 MCP 工具，全部唯讀，全部只打台灣政府的公開資料庫。
...
claude mcp add taiwan-legal-db --scope project --cwd "C:\path\to\mcp-taiwan-legal-db-enhanced" -- `
  "C:\path\to\mcp-taiwan-legal-db-enhanced\.venv\Scripts\python.exe" `
  -m mcp_server.server
```
...
└── mcp_server/
    ├── __init__.py
    ├── server.py          # FastMCP 入口 — 定義 11 個 @mcp.tool() function
...
- 回報問題：[GitHub Issues](https://github.com/AXK1990/mcp-taiwan-legal-db-enhanced/issues)
